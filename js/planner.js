...
//
// ✅ 返却
// - plan[*].blocks.* は {name,tsp,blocks}
// - leftovers は {carb,protein,mineral} それぞれ [{name, remainBlocks}]
// - shortageBlocks は {carbTsp, proteinTsp, mineralTsp}
...
// ミネラルミックス（新食材は混ぜない：mineralBaseのみで構成）
...
export function generateWeeklyPlanWithInventory(customConfig = {}){
  ...
  for(const day of cfg.days){
    ...
    for(let mealNo=1; mealNo<=cfg.mealsPerDay; mealNo++){
      ...
      for(const p of proAlloc.picks) usedTodayProtein.add(p.name);
  ...
  // ✅ leftovers（UIが読む remainBlocks だけを返す）
  const leftovers = {
    carb: inv.carb.map(x=>({
      name: x.name,
